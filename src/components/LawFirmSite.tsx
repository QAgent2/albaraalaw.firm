...
const SectionHeading = ({ label, title, subtitle, centered, dark, number }: { label: string, title?: string, subtitle?: string, centered?: boolean, dark?: boolean, number?: string }) => {
  return (
    <header className={cn("mb-28", centered && "text-center")}>
      ...
      >
        {number && <span className="text-[10px] font-serif italic text-brand-gold/60 mr-4 tracking-widest">{number}</span>}
        <div className="w-6 h-[1px] bg-brand-gold/30" />
        <span className="text-[9px] tracking-[0.6em] uppercase font-bold text-brand-gold font-accents" aria-hidden="true">{label}</span>
        <div className="w-6 h-[1px] bg-brand-gold/30" />
      </motion.div>
      {title && (
        <motion.h2 
          ...
          className={cn(
            "text-4xl md:text-5xl lg:text-7xl font-serif mb-10 leading-[1.1] font-light italic tracking-tight",
            dark === false ? "text-brand-navy" : "text-brand-white"
          )}
        >
          {title}
        </motion.h2>
      ...
      {subtitle && (
        <motion.p 
          ...
          className={cn(
            ...
            dark === false ? "text-brand-navy/60" : "text-brand-white/40"
          )}
        >
          {subtitle}
        </motion.p>
      )}
      <div className={cn("w-20 h-px bg-brand-gold/30", centered && "mx-auto")} />
    </header>
  );
};
...
const Navbar = () => {
  ...
  return (
    <nav className={cn(
      ...
      isScrolled ? "h-20 bg-brand-navy/80 backdrop-blur-xl border-b border-white/[0.05]" : "h-28 bg-transparent pt-4"
      ...
      <div className="max-w-[1600px] mx-auto h-full flex items-center justify-between">
        <a href="#hero" className={cn(
          ...
          <span className="font-serif text-3xl tracking-[0.25em] uppercase text-brand-gold leading-none">البراء</span>
        ...
        <div className="hidden lg:flex items-center gap-12">
          <div className="flex gap-12">
            {menuItems.map((item) => (
              <a 
                key={item.href} 
                href={item.href}
                className="text-[10px] uppercase tracking-[0.3em] font-bold text-brand-white/50 hover:text-brand-gold transition-all duration-300 relative group/link"
              >
                {t(item.name)}
                <span className="absolute -bottom-2 left-0 w-0 h-[1px] bg-brand-gold transition-all duration-500 group-hover/link:w-full" />
              </a>
            ))}
          </div>
          ...
          <button 
            onClick={toggleLang}
            className="flex items-center gap-2 text-[9px] uppercase tracking-[0.3em] text-brand-gold/60 hover:text-brand-white transition-colors cursor-pointer font-bold"
            aria-label="Toggle Language"
          >
            {lang === 'ar' ? 'English' : 'العربية'}
          </button>
          <a href="#contact" className="px-8 py-3 bg-brand-gold/10 border border-brand-gold/30 text-brand-gold text-[10px] uppercase tracking-[0.3em] hover:bg-brand-gold hover:text-brand-navy transition-all duration-500 font-bold glass-card">
        ...
        {/* Mobile Toggle */}
        <button 
          className="lg:hidden text-brand-gold p-2 transition-transform hover:scale-110 active:scale-95"
      ...
      <div className="max-w-[1600px] mx-auto px-8 lg:px-16 w-full relative z-10">
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-12 items-center">
          ...
          <div className="col-span-12 lg:col-span-8">
            ...
            >
              <div className="flex items-center gap-6 mb-12">
                ...
                />
                <span className="text-[11px] tracking-[0.5em] uppercase font-bold text-brand-gold/80 font-accents">
              ...
              <h1 className="text-6xl sm:text-8xl lg:text-[110px] font-serif leading-[0.85] mb-12 text-brand-white relative">
                ...
                <div className="relative inline-block">
                  <span className="font-bold tracking-tighter block gold-gradient-text drop-shadow-[0_10px_30px_rgba(201,162,74,0.3)] pb-4">
                    {t({ ar: 'السيادة القانونية.', en: 'Legal Supremacy.' })}
                  </span>
                  {/* Subtle Light Reflection under text */}
                  <div className="absolute -bottom-2 left-0 w-full h-8 bg-brand-gold/10 blur-xl -z-10" />
                </div>
              </h1>
              ...
              <motion.p 
                ...
                className="max-w-xl text-xl lg:text-2xl leading-relaxed text-brand-white/40 font-light italic mb-16 border-s border-white/5 ps-10"
              >
                {t({ 
              ...
              >
                <MagneticButton onClick={() => window.location.href = '#contact'}>
                  <div className="group bg-brand-gold text-brand-navy px-14 py-6 text-[11px] uppercase tracking-[0.3em] font-bold hover:bg-brand-white transition-all duration-500 flex items-center gap-5 shadow-2xl shadow-brand-gold/20">
          ...
          <div className="col-span-12 lg:col-span-4 lg:ps-12 flex flex-col gap-12 border-s border-white/[0.03] py-12">
            {[
              { val: { ar: '$20B+', en: '$20B+' }, lbl: { ar: 'أصول مُدارة بقيمة', en: 'Capital Managed' }, icon: <Shield size={16} className="text-brand-gold/40" /> },
              { val: { ar: 'التميز السيادي', en: 'Sovereign Distinction' }, lbl: { ar: 'اعتماد دولي', en: 'International Vetting' }, icon: <Globe size={16} className="text-brand-gold/40" /> },
            ].map((stat, i) => (
              <motion.div 
                key={i}
                initial={{ opacity: 0, x: 20 }}
              ...
              >
                 <div className="relative z-10 flex items-start gap-5">
                    ...
                    <div>
                      <div className="text-3xl font-serif text-brand-gold mb-3 italic tracking-tight">{t(stat.val)}</div>
...
const About = () => {
  const { t } = useLang();
  return (
    <section id="about" className="py-56 bg-brand-navy relative overflow-hidden">
      {/* Decorative Accents */}
      ...
      <div className="max-w-[1600px] mx-auto px-8 lg:px-16 grid grid-cols-1 lg:grid-cols-12 gap-24 items-center relative z-10">
        <div className="lg:col-span-6 relative order-2 lg:order-1">
          <motion.div 
            ...
            className="relative"
          >
            <figure className="aspect-[4/5] lg:aspect-[3/4] overflow-hidden shadow-2xl border border-white/5 relative group">
              <img 
                referrerPolicy="no-referrer"
                loading="lazy"
                src="https://images.unsplash.com/photo-1497366216548-37526070297c?q=80&w=2069&auto=format&fit=crop" 
                alt="Elite Law Firm Headquarters" 
                className="w-full h-full object-cover grayscale brightness-50 hover:grayscale-0 hover:brightness-100 transition-all duration-[2s] group-hover:scale-105"
              />
              <div className="absolute inset-0 bg-brand-navy/30 mix-blend-multiply transition-opacity group-hover:opacity-0" />
              
              <div className="absolute -bottom-12 -right-12 w-80 bg-[#081a33] p-14 hidden xl:block shadow-3xl border border-brand-gold/20">
                <motion.div
                  initial={{ opacity: 0, y: 20 }}
                  whileInView={{ opacity: 1, y: 0 }}
                  transition={{ delay: 0.5, duration: 1 }}
                >
                  <span className="text-[10px] tracking-[0.4em] uppercase font-bold text-brand-gold mb-6 block font-accents">Our Core Doctrine</span>
        ...
        <article className="lg:col-span-6 order-1 lg:order-2 lg:ps-12">
          ...
          <motion.div 
            ...
            className="space-y-10 text-brand-white/50 font-light leading-relaxed text-xl"
          >
            <blockquote className="border-s-[3px] border-brand-gold ps-10 py-2 italic text-brand-white/80">
              {t({ 
            ...
            <div className="ps-10 pt-8 flex items-center gap-10">
               <div className="flex flex-col">
                  <span className="text-4xl font-serif text-brand-white mb-1 italic">30+</span>
                  <span className="text-[10px] tracking-widest uppercase font-bold text-brand-gold">Years Experience</span>
               </div>
               <div className="w-px h-12 bg-white/10" />
               <div className="flex flex-col">
                  <span className="text-4xl font-serif text-brand-white mb-1 italic">Global</span>
                  <span className="text-[10px] tracking-widest uppercase font-bold text-brand-gold">Jurisdiction</span>
               </div>
            </div>
          </motion.div>
        </article>
...
const Lawyers = () => {
  const { t } = useLang();
  return (
    <section id="lawyers" className="py-56 bg-brand-gray relative overflow-hidden">
      ...
      <div className="max-w-[1600px] mx-auto px-8 lg:px-16 relative z-10">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-16 lg:gap-24">
          {LAWYERS.map((lawyer, i) => (
            ...
            >
              <div className="relative mb-12 bg-brand-navy overflow-hidden shadow-2xl">
                 ...
                 {/* Decorative Overlay */}
                 <div className="absolute inset-0 border-[1px] border-white/10 m-6 pointer-events-none transition-all duration-700 group-hover:m-0 group-hover:border-brand-gold/40" />
                 ...
                 <div className="absolute bottom-0 left-0 right-0 p-10 translate-y-4 group-hover:translate-y-0 transition-transform duration-700">
                    <div className="flex flex-col gap-3">
                       <span className="text-[9px] tracking-[0.5em] uppercase font-bold text-brand-gold/80 font-accents">{t(lawyer.role)}</span>
                       <h3 className="text-3xl font-serif text-brand-white italic leading-tight mb-2">{t(lawyer.name)}</h3>
                       <div className="w-12 h-[1px] bg-brand-gold/40 transition-all duration-700 group-hover:w-full" />
                    </div>
                 </div>
              </div>
        ...
        <motion.div 
          ...
          className="mt-32 p-14 border border-brand-navy/5 bg-black/5 flex flex-col md:flex-row items-center justify-between gap-12"
        >
          <div className="flex items-center gap-8">
            <div className="w-14 h-14 rounded-full border border-brand-gold/30 flex items-center justify-center text-brand-gold">
              <Globe size={20} />
            </div>
            <div>
          ...
          <MagneticButton as="div">
            <a href="#contact" className="px-10 py-5 bg-brand-navy text-brand-white text-[10px] tracking-[0.4em] uppercase font-bold hover:bg-brand-gold hover:text-brand-navy transition-all duration-500 shadow-xl shadow-brand-navy/10">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-px bg-white/5 border border-white/5 shadow-3xl">
             {PRACTICES.map((p, i) => (
                <article key={p.id} className="p-14 lg:p-16 transition-all duration-700 bg-brand-navy hover:bg-white/[0.03] border border-white/5 group">
                   <div className="text-brand-gold mb-10 group-hover:scale-110 transition-transform origin-left opacity-60 group-hover:opacity-100">
                      {getIcon(p.icon)}
                   </div>
                   <h3 className="text-3xl font-serif italic text-brand-white mb-8 group-hover:text-brand-gold transition-colors">{t(p.title)}</h3>
                   ...
                   <div className="space-y-4 pt-8 border-t border-white/5">
                      {p.capabilities.map((cap, j) => (
                         <div key={j} className="flex items-center gap-4 group/item">
                            <Plus size={10} className="text-brand-gold/40 group-hover/item:text-brand-gold transition-colors" />
...
const Philosophy = () => {
    const { t } = useLang();
    return (
      <section id="why-us" className="py-56 bg-brand-navy relative overflow-hidden">
          ...
          <div className="max-w-[1600px] mx-auto px-8 lg:px-16 relative z-10">
            <div className="grid grid-cols-1 lg:grid-cols-12 gap-24 items-center">
              <div className="lg:col-span-4">
                ...
                />
                <p className="text-brand-white/40 font-light italic leading-loose text-lg">
                  {t({ 
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-px bg-white/5 border border-white/5">
                  {WHY_US.map((item, i) => (
                    <motion.div 
                      ...
                      className="p-16 bg-white/[0.01] border border-white/5 transition-all duration-500"
                    >
                      <h3 className="text-2xl font-serif text-brand-gold mb-8 italic">{t(item.title)}</h3>
                      <p className="text-brand-white/50 font-light leading-relaxed italic text-lg">{t(item.desc)}</p>
                    </motion.div>
                  ))}
                </div>
              </div>
...
const Testimonials = () => {
    ...
    return (
      <section id="testimonials" className="py-56 bg-brand-navy relative overflow-hidden">
          ...
          <div className="absolute top-0 left-1/2 -translate-x-1/2 w-32 h-[1px] bg-brand-gold/30" />
          ...
          <div className="max-w-7xl mx-auto px-8 lg:px-16 relative z-10">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-16 lg:gap-24">
               {TESTIMONIALS.map((test, i) => (
                  ...
                  >
                     <Quote className="absolute top-10 right-10 text-brand-gold/10" size={60} />
...
const FAQ = () => {
    const { t } = useLang();
    return (
      <section id="faq" className="py-56 bg-brand-navy relative overflow-hidden">
        <div className="max-w-4xl mx-auto px-8 lg:px-16">
          <SectionHeading centered dark number="VII" label={t({ ar: 'الإحاطة المعرفية', en: 'Strategic FAQ' })} title={t({ ar: 'تساؤلات النخبة', en: 'Inquiries of Intent' })} />
          <div className="space-y-6">
             {FAQS.map((faq, i) => {
               const [isOpen, setIsOpen] = useState(i === 0);
               return (
                <div key={i} className="group overflow-hidden border border-white/5 bg-white/[0.02]">
                  <button onClick={() => setIsOpen(!isOpen)} className="w-full p-10 lg:p-12 flex items-center justify-between text-left transition-all duration-500 hover:bg-white/[0.04]">
                    <span className={cn("text-2xl md:text-3xl font-serif italic transition-colors duration-500", isOpen ? "text-brand-gold" : "text-brand-white")}>{t(faq.q)}</span>
                    <div className={cn("w-12 h-12 rounded-full border border-white/10 flex items-center justify-center transition-all duration-500", isOpen && "bg-brand-gold text-brand-navy rotate-45")}>
                       <Plus size={18} />
                    </div>
                  </button>
                  <AnimatePresence>
                    {isOpen && (
                      ...
                      >
                        <div className="px-12 pb-14 pt-4 text-xl text-brand-white/40 font-light leading-relaxed italic border-t border-white/5">
                          {t(faq.a)}
                        </div>
                      </motion.div>
                    )}
...
const Briefings = () => {
    const { t } = useLang();
    return (
      <section id="briefings" className="py-56 bg-brand-navy relative overflow-hidden">
        {/* Subtle Decorative Elements */}
        <div className="absolute top-1/2 left-0 w-64 h-64 bg-brand-gold/5 blur-[120px] -translate-x-1/2" />
        ...
        <div className="max-w-7xl mx-auto px-6 relative z-10">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-16">
                {BRIEFINGS.map((entry, i) => (
                    <article key={i} className="group bg-white/[0.01] p-16 border border-white/5 relative cursor-pointer hover:bg-white/[0.03] transition-all duration-700">
                        <div className="flex items-center gap-4 text-[10px] tracking-widest font-bold text-brand-gold opacity-60 mb-8 font-accents">
                            <Calendar size={12} strokeWidth={3} />
                            <span className="uppercase">{entry.date}</span>
                        </div>
                        <h3 className="text-3xl font-serif mb-10 text-brand-white leading-tight group-hover:text-brand-gold transition-colors duration-500 italic">
                            {t(entry.title)}
                        </h3>
                        <div className="flex items-center gap-4 text-brand-white/30 group-hover:text-brand-gold group-hover:translate-x-2 transition-all duration-700">
...
const Contact = () => {
  ...
  return (
    <section id="contact" className="py-56 bg-brand-navy relative overflow-hidden">
      {/* Decorative Dark Gradients */}
      <div className="absolute inset-0 bg-gradient-to-b from-black/60 via-transparent to-black/60 opacity-40" />
      ...
      <div className="max-w-[1600px] mx-auto px-8 lg:px-16 grid grid-cols-1 lg:grid-cols-2 gap-32 relative z-10">
        <div>
          ...
          />
          <p className="text-xl lg:text-2xl text-brand-white/40 font-light italic leading-loose mb-16 border-s border-brand-gold/20 ps-10">
            {t({ 
          ...
          <div className="space-y-10">
             {[
               ...
               <div key={i} className="flex items-start gap-10 group">
                  <div className="w-16 h-16 flex items-center justify-center border border-white/5 bg-white/[0.02] text-brand-gold group-hover:bg-brand-gold group-hover:text-brand-navy transition-all duration-500 shadow-sm glass-card">
                    {item.icon}
                  </div>
                  <div>
                    <div className="text-[10px] tracking-[0.3em] uppercase font-bold text-white/20 mb-2">{t(item.label)}</div>
                    <div className="text-2xl font-serif text-brand-white italic group-hover:text-brand-gold transition-colors">{t(item.value)}</div>
                  </div>
               </div>
             ))}
          </div>
        ...
        <div className="p-12 lg:p-20 bg-[#081a33] border border-white/5 relative shadow-3xl overflow-hidden min-h-[650px] flex flex-col glass-card">
           <div className="absolute top-0 left-0 w-full h-[1px] bg-gradient-to-r from-transparent via-brand-gold to-transparent" />
           ...
           <AnimatePresence mode="wait">
             {formStatus === 'success' ? (
               ...
               >
                 <div className="w-24 h-24 rounded-full border border-brand-gold/20 flex items-center justify-center text-brand-gold relative">
                   <ShieldCheck size={48} />
                   <div className="absolute inset-0 bg-brand-gold/5 rounded-full blur-xl" />
                 </div>
                 ...
                 <button 
                   ...
                   className="text-[10px] tracking-widest uppercase font-bold text-brand-gold border-b border-brand-gold/20 pb-2 hover:text-brand-white hover:border-brand-white transition-all cursor-pointer"
                 >
               ...
               >
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-12">
                    <div className="space-y-4">
                      <label className="text-[10px] tracking-widest uppercase font-bold text-brand-gold/60">{t({ ar: 'الاسم الكامل', en: 'Formal Name' })}</label>
                      <input required type="text" className="w-full bg-white/[0.02] border-b border-white/10 py-5 text-white focus:outline-none focus:border-brand-gold transition-all font-serif italic text-2xl placeholder:opacity-20 outline-none" placeholder="..." />
                    </div>
                    <div className="space-y-4">
                      <label className="text-[10px] tracking-widest uppercase font-bold text-brand-gold/60">{t({ ar: 'رقم الاتصال', en: 'Secure Contact' })}</label>
                      <input required type="text" className="w-full bg-white/[0.02] border-b border-white/10 py-5 text-white focus:outline-none focus:border-brand-gold transition-all font-serif italic text-2xl placeholder:opacity-20 outline-none" placeholder="+" />
                    </div>
                  </div>
                  <div className="space-y-4">
                    <label className="text-[10px] tracking-widest uppercase font-bold text-brand-gold/60">{t({ ar: 'البريد الإلكتروني', en: 'Confidential Email' })}</label>
                    <input required type="email" className="w-full bg-white/[0.02] border-b border-white/10 py-5 text-white focus:outline-none focus:border-brand-gold transition-all font-serif italic text-2xl placeholder:opacity-20 outline-none" placeholder="@" />
                  </div>
                  <div className="space-y-4">
                    <label className="text-[10px] tracking-widest uppercase font-bold text-brand-gold/60">{t({ ar: 'ملخص الإحاطة الإستراتيجي', en: 'Strategic Brief' })}</label>
                    <textarea required rows={4} className="w-full bg-white/[0.02] border-b border-white/10 py-5 text-white focus:outline-none focus:border-brand-gold transition-all resize-none font-serif italic text-2xl outline-none" />
                  </div>
                  <MagneticButton as="div" className="w-full pt-6">
                    <button type="submit" className="w-full py-8 bg-brand-gold text-brand-navy text-[11px] uppercase tracking-[0.4em] font-bold hover:bg-brand-white transition-all duration-700 shadow-xl cursor-pointer">
      ...
      >
        <div className="relative overflow-hidden p-20">
          <motion.div 
            ...
            animate={{ scaleY: 1 }}
            transition={{ duration: 1.5, ease: [0.19, 1, 0.22, 1] }}
            className="absolute left-0 top-0 w-[1px] h-full bg-brand-gold/30 origin-top"
          />
          <div className="text-center relative">
            <motion.div 
              ...
              animate={{ opacity: 1, y: 0 }}
              transition={{ delay: 0.5, duration: 1 }}
              className="text-[10px] tracking-[0.8em] uppercase font-bold text-brand-gold/60 mb-6 font-accents"
            >
